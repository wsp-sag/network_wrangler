...
ServiceID = Annotated[
    ...
    Field(
        None,
        description="Uniquely identifies a set of dates when service is available for one or more routes.",
    ),
]
...
HeadwaySecs = Annotated[
    ...
    Field(
        None,
        ge=1,
        description="Time, in seconds, between departures from the same stop (headway) for the trip, during the time interval specified by start_time and end_time. Multiple headways may be defined for the same trip, but must not overlap. New headways may start at the exact time the previous headway ends.",
    ),
]
...
ShapeDistTraveled = Annotated[
    ...
    Field(
        None,
        ge=0,
        description="Actual distance traveled along the shape from the first shape point to the point specified in this record.",
    ),
]
...
Timezone = Annotated[
    ...
    Field(
        "",
        description="The timezone from https://en.wikipedia.org/wiki/List_of_tz_database_time_zones.",
    ),
]
